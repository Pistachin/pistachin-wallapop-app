...
const ItemsList = ({ isComplete }) => {
  ...
  return (
    <div className={classnames(isComplete ? completeClasses : favouritesClasses)}>
      {
        itemsList.map((item, i) => {
          ...
          return (
            ...
            <div className={classnames(isComplete ? itemCompleteClasses : itemFavouritesClasses)} key={`${i}-item`}>
              <div className={classnames(
                ...
                <img className='rounded-md max-h-full' src={item.image} alt=''/>
              </div>
              <div className={classnames(
                'flex',
                'flex-col',
                ...
                { isComplete && <p className='text-lg font-semibold'>{item.price}€</p> }
                <p className={classnames('text-gray-800', { 'font-semibold': favResult })}>{item.title}</p>
                {
                  isComplete &&
                  <React.Fragment>
                    ...
                    <a className='text-sm font-light text-gray-800' href={`mailto:${item.email}?subject=${item.title} - ${item.price}`}>{item.email}</a>
                  </React.Fragment>
                }
              </div>
              <button
